...
export function EmailRow ({ dispatch, email }) {
  const [editable, setEditability] = React.useState(!email)
  ...
  return (<div className={`mt-8 sm:mt-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-t sm:border-gray-200 sm:px-6 sm:py-5 ${editable && 'h-32'}`}>
    ...
    <div className={'relative'}>
        ...
        <Transition show={editable} enter='transition ease-in duration-1000 transform absolute' enterFrom='opacity-0' enterTo='opacity-100' leave='transition ease-out duration-500 transform' leaveFrom='opacity-100' leaveTo='opacity-0 '>
          <div className='w-full'>
            ...
            <div className="mt-1 rounded-md shadow-sm">
              <input id="email" className="db border-box w-100 measure-narrow ba b--black-20 pa3 br2 mb2 " placeholder="Their email..." value={email} aria-invalid="true" aria-describedby="email" type='email'
                onChange={(e) =>
                  dispatch({ type: 'EMAIL_UPDATED', payload: e.target.value })} />
      ...
      cursor-pointer sm:col-span-2 " onClick={() => setEditability(true)}>
          <svg className="inline-block h-5 w-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg> <p className='pl-1 pb-1 inline-block'>{email}</p></dd></Transition>
      </>
