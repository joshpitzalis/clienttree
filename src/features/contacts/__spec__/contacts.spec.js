...
import { MergeManager } from '../components/ConflictScreen';
import {
  parseContacts,
  handleContactSync,
  findDuplicates,
...
describe('contacts', () => {
  ...
  it('no emails should not count as matches in findDuplicates', () => {
    const existingContacts = [
      {
        activeTaskCount: 1,
        lastContacted: 34,
    ...
    const newContacts = [
      { name: 'abbey', email: 'abbey@example.com' },
      { name: 'Donna', email: 'donna@example.com' },
      { name: '', email: 'donnax@example.com' },
      { name: 'Donnax', email: '' },
    ];

    // name match or email match
    expect(findDuplicates(existingContacts, newContacts)).toHaveLength(1);
    const duplicates = [
      {
        name: 'abbey',
        activeTaskCount: 1,
  ...
  it.skip('add a prmopt t let people know it can take several minutes', () =>
    false);
});

describe('conflict Box', () => {
  ...
  it.skip('does not create a duplicate if the new and existing fields match', () =>
    false);
  it.skip('lets you keep existing details for conflicts', () => false);
  it.skip('lets you overwrite with new details', () => false);
  it.skip('throws an error if there is a problem merging contacts', () =>
    false);
  ...
  it.skip('lets you merge details for conflicts', () => {
    const duplicates = [
      {
        activeTaskCount: 1,
        lastContacted: 34,
